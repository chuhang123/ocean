...
angular.module('webappApp')
    .controller('MyworkHoldonIndexCtrl', ['$scope', 'configService', function($scope, configService) {
        $('#myworkHoldonIndexApplyUnit').combobox({
            data: [{
                "applyId": 1,
                "text": "赛制科技有限公司"
            }, {
        ...
        $('#myworkHoldonIndexTable').datagrid($scope.datagridConfig);

        //获取分页配置
        configService.getPaginationConfig(function(paginationConfig) {
            $scope.paginationConfig = paginationConfig;
        });
        $('#myworkHoldonIndexTable').datagrid('getPager').pagination($scope.paginationConfig);
