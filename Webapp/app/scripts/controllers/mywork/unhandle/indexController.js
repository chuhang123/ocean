...
angular.module('webappApp')
    .controller('MyworkUnhandleIndexCtrl', ['$scope', 'configService', function($scope, configService) {
        $('#myworkUnhandleIndexApplyUnit').combobox({
            data: [{
                "applyId": 1,
                "text": "赛制科技有限公司"
            }, {
        ...
        $('#myworkUnhandleIndexTable').datagrid($scope.datagridConfig);

        //获取分页配置
        configService.getPaginationConfig(function(paginationConfig) {
            $scope.paginationConfig = paginationConfig;
        });
        $('#myworkUnhandleIndexTable').datagrid('getPager').pagination($scope.paginationConfig);
